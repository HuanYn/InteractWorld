...
class CausalInferencePipeline(torch.nn.Module):
    ...
    def generate_next_block(self, noise_block: torch.Tensor):
        ...
        """
        streaming_solver = getattr(self.args, "streaming_solver", "renoise")
        if streaming_solver not in ("renoise", "flow_euler"):
            raise ValueError(f"unsupported streaming_solver: {streaming_solver!r}")
        if streaming_solver == "flow_euler" and self.pyramid_sample_ratio is not None:
            raise ValueError("flow_euler streaming does not support pyramid sampling")
        ...
        for index, current_timestep in enumerate(self.denoising_step_list):
            ...
            if streaming_solver == "flow_euler":
                # This flow model predicts noise - clean. Integrate over the
                # selected schedule's actual sigma intervals, not adjacent
                # entries of the scheduler's 1000-step training table.
                next_timestep = (
                    self.denoising_step_list[index + 1]
                    if index + 1 < len(self.denoising_step_list) else 0
                )
                sigma = torch.as_tensor(
                    current_timestep, device=noisy_input.device, dtype=torch.float32
                ) / 1000.0
                next_sigma = torch.as_tensor(
                    next_timestep, device=noisy_input.device, dtype=torch.float32
                ) / 1000.0
                noisy_input = (
                    noisy_input.float() + (next_sigma - sigma) * flow_pred.float()
                ).to(dtype=noisy_input.dtype)
