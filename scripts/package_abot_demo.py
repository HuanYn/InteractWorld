...
def package(source: Path, output: Path, *, project: Path | None = None) -> dict:
    ...
    (output / 'README.txt').write_text(
        ...
        '若含 .inputs.mp4 展示版：上方48像素保留首帧照片、文字提示与seed；总尺寸832×528，主体画面不缩放、不裁剪。\n'
        '新split_hud布局：左下WASD、右下方向箭头（I↑ J← K↓ L→），仅两个半透明HUD区域覆盖画面；旧inline版本的按键仍位于顶部，以显示收据的key_layout为准。\n'
        '输入栏和HUD是导出后添加的信息，不会送回模型，也不参与原始画面评测；保留未注释原视频。重编码可能产生普通压缩差异。\n'
