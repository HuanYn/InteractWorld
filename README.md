...
Creator 补齐了任意历史版本对比、对齐播放、输入差异及JSON导出，并为视觉观察加入上版原视频参考和保守证据约束。双视频GPU复验已尝试，但两次冷加载超过480秒，尚无新的视觉结论，仍保留人工审核。附有[32条可复现的指令契约用例](docs/creator-cases.md)，不将软件用例通过率当作模型或视频成功率。
