...
from utils.action_alignment import reset_missing_action_adapter
...
class WanDiffusionWrapper(torch.nn.Module):
    ...
    def __init__(
        ...
        if is_causal:
            ...
            model_init_kwargs.pop("output_loading_info", None)
            self.model, loading_info = WanModel.from_pretrained(
                model_name,
        ...
        if not is_causal:
            reset_missing_action_adapter(self.model, loading_info.get("missing_keys", ()))
    ...
    def forward(
        ...
        reference_kwargs = (
            {"ref_latents": ref_latents, "ref_mask": ref_mask}
            if self.is_causal
            else {}
        )
        if training_attention_mode == "bidirectional":
