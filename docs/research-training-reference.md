...
本页保留2026-09-12源码版本的训练入口与环境参考，便于查阅已有实验实现。**其中“当前／尚未”等状态只代表当时记录**，最新展示选择和失败结论以[项目README](../README.md)及[固定展示说明](demo-window6-v1.md)为准。不要因为这里列出连续命令就自动启动后续因果或LongForcing训练；它们不是运行固定Action Demo的前置步骤，也未通过长期生成质量验收。

所有命令都需要操作方自行满足当前GPU、存储、许可和预算条件；示例门禁参数或时间戳不是授权。本次发布没有执行这些GPU命令，也未重新验证训练/生成性能。脚本默认与历史超参数不等于推荐最优配置。
