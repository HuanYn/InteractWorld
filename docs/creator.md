...
补齐后的检查会沿版本关系查找原计划的已完成视频（可识别失败计划的成功重试），把它作为 `reference_video_path`，与当前版各取最多8帧。每条证据区分 `current/reference`，时间戳对应各自视频；收到对照文件与模型实际使用对照分开记录。用户原始请求也会传入待检查目标，防止模型摘要遗漏“全程／缩短”等约束。相对目标缺对照时不调用模型，直接要求人工判断。持续运动、停止或精确时长即使有对照仍不靠抽帧验收；人物头部姿态不能作为镜头俯仰证据。规则降级保留原始模型判断和原因，重复观察保留先前记录。这些补齐项已通过CPU接口/规则检查，**尚未完成新的真实双视频VLM复验**；视觉修订保持关闭。
