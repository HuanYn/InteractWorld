...
2026-09-26较早阶段的实机CPU接口检查：显式规则编排及局部编辑成功，W输入240→240、I输入120→60；该次未提交视频任务，旧会话与4个媒体地址保持可读。当时短时磁盘读取门槛仍超时，未再启动GPU观察。过程记录保存在工作区`reports/interactworld-creator-resume-20260926/`，不随公开源码上传私有配置或日志。

同日用户指定GPU1占用则用GPU2后，视频与VLM部署切为GPU2串行。CPU完整读取已有8.876GB权重耗时225.45秒，并完成目录盘点准备；GPU双视频检查随后成功返回，客户端77.933秒，worker73.176秒（其中加载40.641秒、推理14.383秒），峰值allocated约10.18GiB、reserved约10.77GiB。准备时间不含在77.933秒内，这不是任意冷启动的性能保证。原始模型给出`satisfied/accept`，但混淆人物姿态与镜头、缺少可靠时长对照，规则降为`uncertain/ask_user`。本案例证明运行与拒绝不可靠结论的路径生效，不证明视觉判断准确率、自动纠错或视频质量改善。实际响应和原始负结果保存在`reports/interactworld-creator-gpu2-20260926/`。

随后同卡串行完成新的Qwen请求“保留前进，取消抬头”：计划为240帧W、0帧I，并一次生成/封装新15秒视频（832×480、241帧、16fps），没有CPU恢复旧失败视频。客户端总耗时785.229秒包含编排、加载、生成与轮询；视频任务墙钟698.426秒，峰值allocated约12.57GiB。该样例仍有后段人物模糊/形变，不证明画质改善、全程动作准确或自动修订有效。模型与视频来源、输入和阶段耗时保持分开记录。
...
4. 人工填写满意度及观察依据，可选填移动响应、镜头响应、时序稳定性三个分项；默认均不打分。人工与模型观察独立显示，重复人工评价保留历史。草稿保存在当前浏览器，刷新或保存失败不丢；草稿本身不是已提交评价。点击历史卡“基于此版修改”会将独立修改指令带入上方编辑框，并将编辑基线固定为该版，不会自动请求模型或提交生成。也可使用独立基线选择器：默认动态跟随最新 `ready` 计划，选择历史版后保持固定，提交时绑定页面显示的实际基线；下方对比及接受版本不改变它。也可在已配置视觉模型时请求一次观察；模型建议默认交给人工审核，接受版本始终需要用户操作。
...
当语言/视觉模型与视频生成共用一张卡时，增加 `--serialize-model-and-video`。模型工作期间提交生成或重试返回429且不创建视频任务；视频排队/生成时模型调用同样返回429，提示结束后再提交。规则编排仍可用。这个非阻塞互斥仅覆盖当前服务，物理卡的授权、其他计算进程、共享租约和预算检查仍由原门禁执行，不能用它替代GPU空闲检查。默认不启用，避免改变已配置双卡的旧部署。
